export class InviteCodeScreen extends Phaser.Scene {
    ...
    create(data) {
        ...
        // Add a click event to copy only the last 6 characters of the invite code to the clipboard
        shortAddressText.on('pointerdown', () => {
                ...
                try {
                    ...
                    const response = await fetch(
                        `${import.meta.env.VITE_API_URL || 'http://localhost:3000'}submit-invite`, {
                            ...
                            body: JSON.stringify({
                                code,
                                userAddress: inviterAddress
                            }), // ส่งที่อยู่ผู้เชิญไป backend
                        }
                    );
